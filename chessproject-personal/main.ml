...
(* Assuming that [ft] is on the board, a location [ft] is in check by a piece
   at location [loc]

   IF [loc] and [ft] do not represent the same square on the board
   AND
   IF the color of the piece at [loc] is not equal to the color of [ft]. (If
   there is no piece at [ft] then the color of [ft] is Grey)
   AND
   IF a move by the piece at [loc] to [ft] is a valid move
*)


(* [check_square game ft x y] is a helper function to check and is the boolean
   representation of if the position ([ft].x, [ft].y) of 
...
let new_check game ft =
  let rec new_check_helper game ft x y lst =
    if x = 7 && y = 7 then
      ...
      if check_square game ft x y
      then new_check_helper game ft 0 (y + 1) ({x=x;y=y}::lst)
      else new_check_helper game ft 0 (y + 1) lst
    else
    if check_square game ft x y
    then new_check_helper game ft (x + 1) y ({x=x;y=y}::lst)
    else new_check_helper game ft (x + 1) y lst
  in new_check_helper game ft 0 0 []
...
let checkmate_corner (game : game) (ft : location) : bool =
  if (ft.x = 0 && ft.y = 0) then
    new_check game {x = 1; y = 0} <> [] &&
    new_check game {x = 0; y = 1} <> [] &&
    new_check game {x = 1; y = 1} <> []
  else if (ft.x = 7 && ft.y = 0) then
    new_check game {x = 6; y = 0} <> [] &&
    new_check game {x = 7; y = 1} <> [] &&
    new_check game {x = 6; y = 1} <> []
  else if (ft.x = 0 && ft.y = 7) then
    new_check game {x = 0; y = 6} <> [] &&
    new_check game {x = 1; y = 7} <> [] &&
    new_check game {x = 1; y = 6} <> []
  else 
    new_check game {x = 6; y = 7} <> [] &&
    new_check game {x = 7; y = 6} <> [] &&
    new_check game {x = 6; y = 6} <> []
...
let checkmate_edge_four game ft =
  new_check game {x = (ft.x - 1); y = ft.y} <> [] &&
  new_check game {x = (ft.x - 1); y = (ft.y - 1)} <> [] &&
  new_check game {x = ft.x; y = (ft.y - 1)} <> [] &&
  new_check game {x = (ft.x + 1); y = (ft.y - 1)} <> [] &&
  new_check game {x = (ft.x + 1); y = ft.y} <> []
...
let no_block game ft =
  failwith "Unimplemented"


(** [saving_game game ft a] is a helper function of [no_take], and is the
    boolean representation of whether or not the piece at location [a], 
    which is putting the space represented by [ft] in check, can be captured 
    in a way that would prevent [ft] from continuing to be in check.
    Requires:
    [game] is a valid game
    [ft] is a location that represents a square on [game].board
    [a] is a location that represents a square on [game].board. *)
let saving_take game ft a =
  let rec saving_take_helper game ft a lst =
    match lst with
    | [] -> false
    | h::t -> let taken_piece = game.board.(a.x).(a.y) in 
      (game.board.(a.x).(a.y) <- game.board.(h.x).(h.y));
      (game.board.(h.x).(h.y) <- Empty);
      if new_check game ft = [] then true
      else 
        ((game.board.(h.x).(h.y) <- game.board.(a.x).(a.y));
         (game.board.(a.x).(a.y) <- taken_piece);
         saving_take_helper game ft a t)
  in saving_take_helper game ft a (new_check game a)


(** [dummy_game game] is a copy of is a non physically equal copy of [game].
    Requires:
    [game] is a valid game. *)
let dummy_game game =
  let dummy_board = Array.map (fun x -> Array.copy x) game.board
  in {game with board = dummy_board} 


(** [no_take game ft lst] is the boolean representation of whether or not one 
    the pieces the locations in [lst] that are putting the space [ft] in check
    could be captured in a way that would prevent [ft] from continuing to be in 
    check.
    Requires: 
    [game] is a valid game
    [ft] is a location that represents a space on [game].board
    [lst] is either empty or filled with locations that pieces that are putting
    the space [ft] in check. *)
let rec no_take game ft lst =
  match lst with
  | [] -> true
  | h::t -> if saving_take (dummy_game game) ft h then false else no_take game ft t


let checkmate game ft =
  new_check game ft <> [] && no_block game ft && 
  new_check game ft |> no_take game ft &&
...
(* A piece of color col is in checkmate at location l if and only if
*)
...
let make_move game ft sd = 
  if check_move_basics game ft sd && move_king game ft 
     && new_check game sd <> []
